...
- Insta TOC: auto-generate and update TOC, omit headings; created by simple codeblock, not commands, include HTML or symbols in headings, allows skipping hierarchical steps, support markdown/wikilink in headings, regex exclude headers with shared features, change bullet style of ToC, limit heading level included. May not export well yet. -- this looks pretty nice but doesn't display in a sidebar, not sure what features this has that those don't. The sidebar view might be nice, but also could be more complicated than needed. The question is how much the reader needs to see the ToC, when, and whether they are expected to use it to navigate between parts of the document often, or it mostly serves as an introduction/outline.
- Recent Edits: see changes grouped by day, designed for accumulating script/AI changes in a single location but could be useful for summarizing multiple contributors. This highlights changes not made by the user (in the editor), but perhaps we could highlight/identify the account responsible. Also identifies automated import/sync from outside platforms which could be useful. All files created after the plugin is installed will appear like new modifications, and sync from other devices can trick it. You can exclude certain automated functions from triggering this log.
...
- Tabsdown: create horizontal tabs within notes to nest content, allows embedded tabs. -- The example provided looks super cool and I think this could be a nice way to provide info within a note without doing tons of scrolling, allow quick switching, and provide a sort of outline within notes.
- Tab Panels: similar to above, create "tabs" codeblock and insert as many tabs and content inside as you want. Can add code blocks inside tab panels, but have to switch the indicator to ~~~ which would throw off some templates. Interactivity (e.g. checkbox) is broken in tabs. Footnotes work as long as the reference and definition are included in the same tab block. Queries don't seem to pull from inside tabs, headings won't appear in auto-outlines either.
- Canvas Minimap: provides a clickable mini-map that aids in navigating larger canvases, as they are unbounded and you can lose notes in them. They note this might not be stable.
- Glossary Linker: multi-lingual glossary that tracks forms across morphology, convert highlights into real wikilinks, learn new aliases from use; 6 languages out of the box, -- Real links would clutter your links/backlinks section horrendously. It seems like they used the same morphology matching engine to make linking to files and headers easier in related plugins, i.e. so you don't have to remember the exact wording of the heading/file.
...
- Wayfarer: converts pasted Google Maps links into stops, drag-and-drop to reorder, headers are days of the trip, the map view moves to align with the cursor location in the note, timeline, export to Google My Maps, one KML/note, pulls hours of operation, image thumbnails, and more from Google Maps -- the paired window of map and cursor on text is super interesting. This feature could be cool in other places like parsing Greek, video notes, or diagrams/canvas. Drag and drop timeline is cool. WAIT! Maybe the map view could preview assignments embedded in a canvas and the note is the linear structure of the course. An instructor could add assignment types by working through a simple wizard or linking directly to an assignment they know of. Instead of calculating hour and distances, it would give work hours.
...
- Advanced Maps: photo atlas and route viewer addition to default Maps feature, GPX/GeoJSON/KML/TCX tracks; lists "around" notes which are geographically similar to the current note -- now thats a cool way to discover related notes.

...
- Recent Edits: see changes grouped by day, designed for accumulating script/AI changes in a single location but could be useful for summarizing multiple contributors. This highlights changes not made by the user (in the editor), but perhaps we could highlight/identify the account responsible. Also identifies automated import/sync from outside platforms which could be useful
- 

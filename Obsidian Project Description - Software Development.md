...
#### Obsidian Plugins
- [ZettelFlow](https://community.obsidian.md/plugins/zettelflow): a huge collection of features including an interactive workflow coach VERY similar to what I want for syllabus builder. Allows "derived projects" which may support features like LongForm, in-folder organization. Vault health dashboards that calculate orphans, connectivity, weekly progress reports, tracks "knowledge-debt", MOC builder (doesn't do everything some want, but I'm not sure right now which we need), 3-D knowledge mp

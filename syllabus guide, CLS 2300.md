
Hello ADVANCE Instructor. This guide will walk you through the process of creating your syllabus using the syllabus template prepared by Gateway Seminary faculty. This guide incorporates each element of the syllabus template, plus instructions and advice specific to you as the instructor. The guide will coach you through making edits directly to the syllabus template in order to produce your student-ready syllabus. At the end of this guide, your student-ready syllabus will be output as a separate file for you to share with students and automatically submitted for approval by the ADVANCE office. Depending on the kinds of changes you made, your syllabus may be approved immediately.

Now, let's begin:

The first part of your syllabus (after the name & course code) is the course description. This should give a very general description of the course, but gives you and students a basic idea of what kinds of topics we are trying to cover in this course. 
   ![[CLS 2300 - Exegesis in the New Testament#Course Description]]
- If you would like, you can add to this course description by supplementing your own description below:
  (               )

The next part of your syllabus is the Course Learning Objectives. These can be wordy but describe the specific skills that students should gain in your course. Your syllabus only includes the highest level learning objectives to keep things simple, but you can click any of them to access more information about these and related skills. For example, what do we mean when we say students should demonstrate an "advanced knowledge" of something? As you dive into this learning objective, you will find a list of specific skills which we think constitute "advanced knowledge" of this topic. 
We recognize that some skills must be mastered sequentially and others need to be exercised and refined together. Thus, each learning objective has other objectives which it builds on, towards, or with simultaneously. 
Each learning objective should have instruction, class discussion, reading, and other learning activities that help develop it throughout the course. This short list tells students in precise terms what they should expect to learn from this course.
In general, these high-level learning objectives listed on your syllabus should not require modification in order to properly contextualize a course. Rather, you will need to contextualize your approach to teaching these skills and the specific sub-skills that make up each learning objective.


> [!NOTE] Example
> Have students complete a group discussion and present their findings to the class.


```cornell
%%> Example %% The context of your students will determine how they can best **demonstrate** their advanced knowledge of New Testament books. It is possible to demonstrate this knowledge by passing an exam, writing a paper, giving an oral presentation, teaching a Bible study, composing a song, or drawing a visual outline and graphic representation of the book.
```


![[CLS 2300 - Exegesis in the New Testament#Course Objectives]]



